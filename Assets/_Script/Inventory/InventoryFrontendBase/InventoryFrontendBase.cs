        ...
        where TInventory : InventoryBackend.Inventory
    {
        ...
        protected virtual void InitializeInventoryUI()
        {
            ...
            for (int i = 0; i < inventory.Capacity; i++)
            {
                ...
                inventorySlotInteraction.SetSlot(inventory.GetItemInstanceAt(i));
            }
            
            for (int i = 0; i < inventory.ItemInstances.Count ; i++)
            {
                _slotInteractions[i].SetSlot(inventory.GetItemInstanceAt(i));
            }
        ...
        protected virtual void UpdateSlotUI(int slotIndex)
        {
            ...
            slotInteraction.SetSlot(inventory.GetItemInstanceAt(slotIndex));
        }
        ...
        public virtual ItemInstance.ItemInstance RemoveAllItemsFromSlot(int slotIndex)
        {
            return inventory.RemoveAllItemsFromSlot(slotIndex);
        }

        public virtual void AddItemToEmptySlot(ItemInstance.ItemInstance itemInstance, List<Vector2Int> peakItemStack)
        {
            inventory.AddItemToEmptySlot(itemInstance, peakItemStack);
        }

        public ItemInstance.ItemInstance AddItem(ItemInstance.ItemInstance itemInstance)
        {
            return inventory.AddItem(itemInstance);
        }

        public virtual bool AcceptsItem(ItemInstance.ItemInstance itemInstance)
        {
            return true;
        }
        ...
        private void RenderSlotIcons()
        {
            ...
            foreach (var item in inventory.ItemInstances)
            {
                ...
                var itemSize =item.ItemShape.IconScale;
                if(item.IsRotated)
                {
                    rect.Rotate(0, 0, -90);
                }
                ...
                slotUI.SetDisplay(item.ItemIcon, item.Quantity);
                _slotUIs.Add(slotUI);
            }
        }
